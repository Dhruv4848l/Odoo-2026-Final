...
export const Badge: React.FC<BadgeProps> = ({
  ...
  // Infer variant from status text if variant not explicitly provided
  ...
  if (!variant && status) {
    ...
    } else if (['pending', 'to approve', 'warning', 'unsent', 'draft'].includes(s)) {
      effectiveVariant = s === 'draft' ? 'neutral' : 'warning';
    } else if (['absent', 'refused', 'expired', 'overdue', 'duplicate', 'failed'].includes(s)) {
      effectiveVariant = 'danger';
    } else if (['viewed', 'validated'].includes(s)) {
